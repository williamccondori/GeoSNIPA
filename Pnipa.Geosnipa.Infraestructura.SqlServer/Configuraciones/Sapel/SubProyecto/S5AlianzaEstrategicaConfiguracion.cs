...
public class S5AlianzaEstrategicaConfiguracion : IEntityTypeConfiguration<S5AlianzaEstrategicaEntidad>
{
    public void Configure(EntityTypeBuilder<S5AlianzaEstrategicaEntidad> builder)
    {
        ...
        builder.Property(p => p.AlianzaEstrategicaId).HasColumnName("ALIANZAESTRATEGICA_ID").IsRequired();
        builder.Property(p => p.SubProyectoId).HasColumnName("SUBPROYECTO_ID");
        builder.Property(p => p.Siglas).HasColumnName("SIGLAS");
        builder.Property(p => p.TmDetDesctipcionRolConcurso).HasColumnName("TMDET_DESC_ROLCONCURSO");
        builder.Property(p => p.TmDetDesctipcionCategoriaActividad).HasColumnName("TMDET_DESC_CATEGORIAACTIVIDAD");
